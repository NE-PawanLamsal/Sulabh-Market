...
import 'package:sulabh_market_app/screens/auth/phone_otp_screen.dart';
...
class Auth {
      ...
      if (isLoginUser) {
        ...
        if (result.exists) {
          ...
          registerWithEmail(
            ...
            firstName!,
            lastName!,
            userImage, // Pass the user image URL to the registration function
          );
        }
      }
    } catch (e) {
  ...
  void registerWithEmail(
    ...
    String firstName,
    String lastName,
    String? userImage, // Add the user image URL parameter
  ) async {
    try {
      loadingDialogBox(context, 'Validating details');
      ...
      return users.doc(credential.user!.uid).set({
        ...
        'mobile': '',
        'address': '',
        'user_img': userImage, // Store the user image URL in the database
      }).then((value) async {
        ...
        customSnackBar(
          context: context,
          content: 'Failed to add user to database, please try again $onError',
        );
      });
    } on FirebaseAuthException catch (e) {
      Navigator.pop(context);
